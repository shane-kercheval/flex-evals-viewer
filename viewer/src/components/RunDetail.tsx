...
import { testCasePassed } from '../types'
    ...
    <div>
          ...
          {results.pass_mode === 'per_test_case' && results.per_test_case ? (
            <div>
              ...
              <p className="font-medium text-gray-900 tabular-nums">
                {results.per_test_case.filter(tc => testCasePassed(tc, results.pass_threshold)).length} / {results.per_test_case.length} passing
              </p>
            </div>
          ) : (
            <div>
      ...
      <details className="mt-4">
        ...
        <div className="mt-2 bg-gray-50 rounded-lg border border-gray-200 p-4 text-xs text-gray-600 space-y-2">
          ...
          <p>
            <strong>Total Cost / Tokens</strong> (header) includes both agent response costs and LLM judge costs across all samples.
          </p>
          <p>
            <strong>Per-sample duration</strong> reflects the agent response time only; it does not include check execution or judge evaluation time.
          </p>
          <p>
            <strong>Per-sample cost</strong> reflects the agent response only; judge cost is shown separately within the check detail.
          </p>
        </div>
      </details>
    </div>

...
export default function SyntaxJson({ data, className = '' }: SyntaxJsonProps) {
  if (data === undefined) {
    return (
      <pre className={`text-xs bg-gray-50 p-2.5 rounded overflow-x-auto whitespace-pre-wrap text-gray-400 ${className}`}>
        undefined
      </pre>
    )
  }

  // Render primitive strings as plain text to preserve readability
  // (JSON.stringify would add quotes and escape newlines)
